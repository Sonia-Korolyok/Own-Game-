...
export const topics: Topic[] = [
  {
    ...
    questions: [
      ...
      { price: 500, question: "Что такое прототипное наследование?", answer: "Механизм, при котором объекты наследуют свойства и методы через __proto__ или Object.create. " +
              "Прототипное наследование — это механизм в JavaScript, при котором объекты наследуют свойства и методы других объектов через цепочку прототипов. Если свойство не найдено в объекте, движок ищет его в прототипе, затем в прототипе прототипа и так далее до null." +
              "аждый объект в JS имеет скрытое свойство [[Prototype]], которое можно увидеть через __proto__ или через Object.getPrototypeOf(obj)." +
              "Это ссылка на другой объект, от которого текущий объект «унаследует» свойства." },
